...
This code adapts and elaborates on the Gromov-Wasserstein distance code of
Flamary and Courty from the Python Optimal Transport library:
    https://pot.readthedocs.io/en/stable/.

The theory is explained in our paper
...
def gromov_wasserstein_asym(C1, C2, p, q, log=True, G0=None):
    if G0 is None:
        G0 = p[:, None] * q[None, :]
    ...
    def f(G):
        loss = gwloss_asym(constC, constCt, hC1, hC1t, hC2, hC2t, G)
        return loss

    def df(G):
        return gwggrad_asym(constC, constCt, hC1, hC1t, hC2, hC2t, G)

    # print "initial GW_loss = ", f(G0)
...
def gromov_wasserstein_asym_loss_only(C1, C2, p, q, G0=None):
    C1t = np.transpose(C1)
    C2t = np.transpose(C2)
    ...
    return gwloss_asym(constC, constCt, hC1, hC1t, hC2, hC2t, G0)
...
""" Code for preprocessing coupling matrices and probability vectors """
...
def log_map_ignore(C0, C1, p, q, G0=None):
    if G0 is None:
        opt_coup, log = gromov_wasserstein_asym(C0, C1, p, q)
    else:
        opt_coup_1, log_1 = gromov_wasserstein_asym(C0,C1,p,q,G0=G0)
        opt_coup_2, log_2 = gromov_wasserstein_asym(C0,C1,p,q)
        if log_1['gw_dist'] < log_2['gw_dist']:
            opt_coup = opt_coup_1
            log = log_1
        else:
            opt_coup = opt_coup_2
    ...
    columns_to_keep = np.argmax(opt_coup, axis=1)
    tmp = np.zeros(opt_coup.shape)
    for i in range(len(opt_coup)):
        tmp[i][columns_to_keep[i]] = opt_coup[i][columns_to_keep[i]]
    # assert abs(np.sum(tmp) - 1.0) < 1e-10
    opt_coup = tmp
    q = np.sum(opt_coup, axis=0)
    ...
    return C0, C1, p, q, vec, opt_coup, log


def log_map(C0, C1, p, q, G0=None):
    opt_coup, log = gromov_wasserstein_asym(C0,C1,p,q,G0=G0)
...
def log_map_compressed(C0, C1, p, q, opt_coup):
    ...
    coup_new, C0_new, C1_new, _, q_new = split_cost_coupling_probabilities(
        opt_coup, C0, C1, p, q)
    perm = 1*(coup_new != 0)
...
def frechet_gradient_compressed(CList,pList,CBase,pBase,budget):
    ...
    for j in range(numC):
        ...
        # special_oc = np.zeros((8, 6))
        # special_oc[2, :] = 1 / 48
        # special_oc[3, :] = 1 / 48
        # special_oc[0, 0] = special_oc[1, 1] = special_oc[4, 2] = 1 / 8
        # special_oc[5, 3] = special_oc[6, 4] = special_oc[7, 5] = 1 / 8
        #
        # if len(pBase) == 6 and len(pList[j]) == 8:
        #     opt_coup, log = gromov_wasserstein_asym(CBase, CList[j], pBase, pList[j], G0=special_oc.transpose())
        # elif len(pBase) == 8 and len(pList[j]) == 6:
        #     opt_coup, log = gromov_wasserstein_asym(CBase, CList[j], pBase, pList[j], G0=special_oc)
        # else:
...
def network_karcher_mean_armijo(CList,pList,maxIter = 50):
    ...
    while Delta > 1e-10*loss_init and counter < maxIter: # This is a pretty arbitrary stopping condition
        ...
        alpha, fc, f_val = line_search_armijo(
            frechet_loss_at, C0, gradient, -gradient, f_val)
        #step size satisfying Armijo condition
        CBase = exp_map(C0, alpha*gradient)
...
# Network Karcher mean with a schedule for changing the step size
...
def network_karcher_mean_armijo_sched_compress(
        CBase, pBase, CList,pList,budget,exploreIter = 20, maxIter = 10):
    # Take exploreIter full gradient steps at first,
    # then do backtracking line search up to maxIter
    # Delta = 1000
    counter = 0
